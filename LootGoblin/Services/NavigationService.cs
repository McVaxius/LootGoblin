...
{
    private enum PositionNavigationKind
    {
        None,
        FlyTo,
        MoveTo,
    ...
    private enum NavigationCommandKind
    {
        None,
        FlyToPosition,
        MoveToPosition,
    ...
    private enum NavigationCommandStartResult
    {
        SendNow,
        Queued,
        Failed,
    ...
    private PositionNavigationKind activePositionNavigationKind = PositionNavigationKind.None;
    private Vector3 activePositionNavigationTarget;
    private NavigationCommandKind activeNavigationCommandKind = NavigationCommandKind.None;
    private Vector3 activeNavigationCommandTarget;
    private NavigationCommandKind pendingNavigationCommandKind = NavigationCommandKind.None;
    private Vector3 pendingNavigationCommandTarget;
    ...
    public void Update()
    {
        if (State != NavigationState.WaitingForNavmesh ||
            pendingNavigationCommandKind == NavigationCommandKind.None)
        {
            return;
        }

        _plugin.VNavIPC.CheckAvailability(logStatus: false);
        if (!_plugin.VNavIPC.IsAvailable)
        {
            ClearPendingNavigationCommand();
            SetState(NavigationState.Error, "vnavmesh not available.");
            return;
        }

        var isReady = _plugin.VNavIPC.TryIsNavReady();
        if (isReady == false)
            return;

        var commandKind = pendingNavigationCommandKind;
        var target = pendingNavigationCommandTarget;
        ClearPendingNavigationCommand();

        _plugin.AddDebugLog("[Navigation] vnavmesh is ready; starting queued navigation.");
        SendNavigationCommand(commandKind, target);
    }

    public bool NotifyVnavCommandRejectedWhileNavmeshBuilding(string text)
    {
        _plugin.VNavIPC.CheckAvailability(logStatus: false);
        if (!_plugin.VNavIPC.IsAvailable)
            return false;
        ...
        if (pendingNavigationCommandKind != NavigationCommandKind.None &&
            State == NavigationState.WaitingForNavmesh)
        {
            return true;
        }

        if (activeNavigationCommandKind == NavigationCommandKind.None)
            return false;

        QueuePendingNavigationCommand(
            activeNavigationCommandKind,
            activeNavigationCommandTarget,
            "vnavmesh rejected the command while navmesh was building");
        _plugin.AddDebugLog($"[Navigation] vnavmesh command rejected while navmesh is building; waiting to retry. Message: {text}");
        return true;
    }
    ...
    public void FlyToPosition(Vector3 position, bool force = false)
    {
        ...
        var startResult = PrepareVNavCommand(NavigationCommandKind.FlyToPosition, position);
        if (startResult != NavigationCommandStartResult.SendNow)
            return;

        SendNavigationCommand(NavigationCommandKind.FlyToPosition, position);
    }

    public void MoveToPosition(Vector3 position)
    {
        if (IsActivePositionNavigation(PositionNavigationKind.MoveTo, position))
            return;

        TargetPosition = position;
        var startResult = PrepareVNavCommand(NavigationCommandKind.MoveToPosition, position);
        if (startResult != NavigationCommandStartResult.SendNow)
            return;

        SendNavigationCommand(NavigationCommandKind.MoveToPosition, position);
    }

    public void StopNavigation(bool clearFlag = false)
    {
        ClearPendingNavigationCommand();
        ClearActiveNavigationCommand();
    ...
    {
        if (State == NavigationState.WaitingForNavmesh &&
            pendingNavigationCommandKind == NavigationCommandKind.FlyToFlag)
        {
            return;
        }

        var startResult = PrepareVNavCommand(NavigationCommandKind.FlyToFlag, Vector3.Zero);
        if (startResult != NavigationCommandStartResult.SendNow)
            return;

        SendNavigationCommand(NavigationCommandKind.FlyToFlag, Vector3.Zero);
    }
    ...
    internal double? GetPlayerXZDistanceToAetheryte(uint aetheryteId)
    {
        var playerPos = Plugin.ObjectTable.LocalPlayer?.Position ?? Vector3.Zero;
        if (playerPos == Vector3.Zero)
            return null;

        if (!TryGetAetheryteWorldPosition(aetheryteId, out var aetherytePosition))
            return null;

        var dx = playerPos.X - aetherytePosition.X;
        var dz = playerPos.Z - aetherytePosition.Z;
        return Math.Sqrt(dx * dx + dz * dz);
    }

    private bool TryGetAetheryteWorldPosition(uint aetheryteId, out Vector3 position)
    {
        ...
        if (_plugin.AetherytePositionDatabase != null)
        {
            var storedPosition = _plugin.AetherytePositionDatabase.GetPosition(aetheryteId);
            if (storedPosition != null)
            {
                position = new Vector3(storedPosition.X, storedPosition.Y, storedPosition.Z);
                return true;
            }
        }

        var estimatedPosition = GetEstimatedAetherytePosition(aetheryteId);
    ...
    private NavigationCommandStartResult PrepareVNavCommand(NavigationCommandKind commandKind, Vector3 target)
    {
        ...
        if (!_plugin.VNavIPC.IsAvailable)
        {
            ClearPendingNavigationCommand();
            SetState(NavigationState.Error, "vnavmesh not available.");
            return NavigationCommandStartResult.Failed;
        }

        var isReady = _plugin.VNavIPC.TryIsNavReady();
        if (isReady == false)
        {
            QueuePendingNavigationCommand(commandKind, target, "navmesh is building");
            return NavigationCommandStartResult.Queued;
        }

        return NavigationCommandStartResult.SendNow;
    }

    private void QueuePendingNavigationCommand(NavigationCommandKind commandKind, Vector3 target, string reason)
    {
        var alreadyPending = IsPendingNavigationCommand(commandKind, target);

        pendingNavigationCommandKind = commandKind;
        pendingNavigationCommandTarget = target;

        if (alreadyPending && State == NavigationState.WaitingForNavmesh)
            return;

        _plugin.AddDebugLog($"[Navigation] Waiting for vnavmesh to finish building the navmesh before {DescribeNavigationCommand(commandKind, target)} ({reason}).");
        SetState(NavigationState.WaitingForNavmesh, BuildWaitingForNavmeshDetail(commandKind, target));
    }

    private bool IsPendingNavigationCommand(NavigationCommandKind commandKind, Vector3 target)
    {
        if (pendingNavigationCommandKind != commandKind)
            return false;

        return commandKind == NavigationCommandKind.FlyToFlag ||
               Vector3.DistanceSquared(pendingNavigationCommandTarget, target)
                   <= PositionCommandReuseTolerance * PositionCommandReuseTolerance;
    }

    private void SendNavigationCommand(NavigationCommandKind commandKind, Vector3 target)
    {
        switch (commandKind)
        {
            case NavigationCommandKind.FlyToPosition:
                TargetPosition = target;
                RememberActivePositionNavigation(PositionNavigationKind.FlyTo, target);
                RememberActiveNavigationCommand(commandKind, target);
                _plugin.VNavIPC.FlyTo(target);
                SetState(NavigationState.Flying, $"Flying to {CommandHelper.FormatVector(target)}...");
                break;

            case NavigationCommandKind.MoveToPosition:
                TargetPosition = target;
                RememberActivePositionNavigation(PositionNavigationKind.MoveTo, target);
                RememberActiveNavigationCommand(commandKind, target);
                _plugin.VNavIPC.MoveTo(target);
                SetState(NavigationState.Flying, $"Moving to {CommandHelper.FormatVector(target)}...");
                break;

            case NavigationCommandKind.FlyToFlag:
                ClearActivePositionNavigation();
                RememberActiveNavigationCommand(commandKind, target);
    ...
    private static string BuildWaitingForNavmeshDetail(NavigationCommandKind commandKind, Vector3 target)
    {
        return commandKind switch
        {
            NavigationCommandKind.FlyToPosition =>
                $"Waiting for vnavmesh to finish building the navmesh before flying to {CommandHelper.FormatVector(target)}...",
            NavigationCommandKind.MoveToPosition =>
                $"Waiting for vnavmesh to finish building the navmesh before moving to {CommandHelper.FormatVector(target)}...",
            NavigationCommandKind.FlyToFlag =>
                "Waiting for vnavmesh to finish building the navmesh before flying to map flag...",
            _ => "Waiting for vnavmesh to finish building the navmesh...",
        };
    }

    private static string DescribeNavigationCommand(NavigationCommandKind commandKind, Vector3 target)
    {
        return commandKind switch
        {
            NavigationCommandKind.FlyToPosition => $"flying to {CommandHelper.FormatVector(target)}",
            NavigationCommandKind.MoveToPosition => $"moving to {CommandHelper.FormatVector(target)}",
            NavigationCommandKind.FlyToFlag => "flying to map flag",
            _ => "navigating",
        };
    }

    private void RememberActiveNavigationCommand(NavigationCommandKind commandKind, Vector3 target)
    {
        activeNavigationCommandKind = commandKind;
        activeNavigationCommandTarget = target;
    }

    private void ClearActiveNavigationCommand()
    {
        activeNavigationCommandKind = NavigationCommandKind.None;
        activeNavigationCommandTarget = default;
    }

    private void ClearPendingNavigationCommand()
    {
        pendingNavigationCommandKind = NavigationCommandKind.None;
        pendingNavigationCommandTarget = default;
    }
    ...
    private bool IsActivePositionNavigation(PositionNavigationKind kind, Vector3 position)
    {
        var commandKind = kind == PositionNavigationKind.FlyTo
            ? NavigationCommandKind.FlyToPosition
            : NavigationCommandKind.MoveToPosition;

        var activeMatch = State == NavigationState.Flying
            && activePositionNavigationKind == kind
            && Vector3.DistanceSquared(activePositionNavigationTarget, position)
                <= PositionCommandReuseTolerance * PositionCommandReuseTolerance;
        var pendingMatch = State == NavigationState.WaitingForNavmesh
            && pendingNavigationCommandKind == commandKind
            && Vector3.DistanceSquared(pendingNavigationCommandTarget, position)
                <= PositionCommandReuseTolerance * PositionCommandReuseTolerance;

        return activeMatch || pendingMatch;
    }

    private void RememberActivePositionNavigation(PositionNavigationKind kind, Vector3 position)
    {
        activePositionNavigationKind = kind;
        activePositionNavigationTarget = position;
    }

    private void ClearActivePositionNavigation()
    {
        activePositionNavigationKind = PositionNavigationKind.None;
        activePositionNavigationTarget = default;
    }
}
