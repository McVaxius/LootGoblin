...
### Added
- **BMR obstacle maps** - Added the saved **Obstacle maps on** switch in Advanced, defaulting to off. Every accepted Start and the end of every configured landing, duty-entry, or finish command batch send the selected state to BossMod Reborn, including character overrides and empty batches. VBM is unaffected; Stop does not restore the setting.
- **Settings search and setup guidance** - Search above the nine settings tabs highlights matching tabs by name, setting or alias, including hidden options. Every search word must match, ignoring case; Clear or closing settings resets the search. Brief tab guidance explains prerequisites, factory defaults and AutoRetainer discard timing without changing automation or defaults.
- **Session Emptor price hints** - Emptor v5+ now supplies one load/login batch of NQ minimum-listing hints for all known marketable maps at the current world, data-center, region, or reachable-regions-plus-Materia scope selected by market travel settings. Ceiling tooltips show price, world/location, age, scope, and session/staleness guidance; a separate **Use** action copies a positive hint without enabling the cart. A global manual refresh has a visible five-minute countdown, one bounded pending follow-up, and no automatic refresh on travel-setting changes.
- **Opt-in Emptor map purchasing** - Marketable map rows expose a default-off cart and per-map gil cap. After inventory, saddlebag, retainer, and gathering sources are exhausted, LootGoblin submits quantity-one Emptor orders, verifies every map in normal inventory, and can prepare up to three maps per trip, capped by remaining configured runs. One trip can therefore spend the configured per-map cap up to three times.
- **Emptor city and install controls** - The Marketboard tab keeps blank/Ul'dah requests compatible with Emptor v1-v3, offers every Emptor v4 city, blocks selected-city requests clearly on older APIs, and shows a large red missing-Emptor warning with copy-repository, `/xlsettings`, and `/xlplugins` buttons.
- **Marketboard travel and party restoration** - The Marketboard settings tab adds sticky-success or rotating world order, optional same-DC/local-region/OCE scopes, supported Lifestream world travel, strict leader/roster checks before disbanding, return-to-start verification, immediate/30-second party re-invites with configurable timeout continuation, and explanatory hover help for every setting, including disabled controls. Batched purchases finish before return and party restoration; only then may a deciphered batch map begin travel.
...
- **Immediate market stop ownership** - Stop, reset, and unload cancel only LootGoblin's recorded Emptor order once, abort only Lifestream travel that LootGoblin successfully started, clear market-search and batch runtime state (including pending batch decipher callbacks), and warn when manual world return or party restoration may still be required.
...
- **Saddlebag map move safety** - Saddlebag retrieval and market-batch storage open `InventoryBuddy`, wait for the addon to become visible and stable, rescan loaded containers, revalidate source/destination slots and counts, issue one move, require exact post-move slot/count confirmation, then close the UI. Two-run batches prefer saddlebag + inventory; three-run batches add a deciphered key item. Disabled saddlebag retrieval uses key item + inventory, and any later purchase/move/decipher failure warns and continues with secured stock.
